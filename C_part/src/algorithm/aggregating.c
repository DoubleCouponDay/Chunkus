...
uint64_t* OR_64(splits* input, int width, int height) {
    ...
    for(int y = 0; y < height; ++y) {
        ...
        while(endindex < width) {
            endindex = endindex + 63;

            if(endindex >= widthindex) {
                endindex = widthindex;
            }
            uint64_t split1slice = getslice(input.splits[0], startindex, endindex, height);
            uint64_t split2slice = getslice(input.splits[1], startindex, endindex, height);
            uint64_t split3slice = getslice(input.splits[2], startindex, endindex, height);
            uint64_t split4slice = getslice(input.splits[3], startindex, endindex, height);
            uint64_t split5slice = getslice(input.splits[4], startindex, endindex, height);
            uint64_t split6slice = getslice(input.splits[5], startindex, endindex, height);
            uint64_t split7slice = getslice(input.splits[6], startindex, endindex, height);
            uint64_t split8slice = getslice(input.splits[7], startindex, endindex, height);
